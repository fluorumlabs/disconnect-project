...
import com.github.fluorumlabs.disconnect.core.internals.ValidatorRegistry;
...
public class Validation {
    public static <T> ValidationResult validate(Class<T> cls, T instance, String fieldName) {
        Internals.VALIDATION_RESULT = new ValidationResult();
        validate_(cls, instance, fieldName);
        return new ValidationResult(Internals.VALIDATION_RESULT);
    }

    @Meta
    private static native <T> void validate_(Class<T> cls, T instance, String fieldName);

    @SuppressWarnings("unchecked")
    private static <T> void validate_(ReflectClass<T> cls, Value<T> instance, Value<String> fieldName) throws ClassNotFoundException {
        if (cls.isPrimitive() || cls.isArray() || cls.getName().startsWith("~")) {
            return;
        }
        ...
        for (ReflectField field : cls.getDeclaredFields()) {
            ...
            if (field.getAnnotation(Valid.class) != null) {
                // cascade
                emit(() -> {
                    if (shouldEmit.get() && instance.get() != null) {
                        Object object = field.get(instance.get());
                        if (object != null) {
                            validate_((Class)object.getClass(), object, null);
                        }
                    }
                });
            }
    ...
    private static <T, A extends Annotation> void emitValidator(AnnotationDescriptor annotation, Class<?> validatorClass, Value<T> o, Value<Boolean> shouldEmit) {
        ...
        MessageInterpolator defaultMessageInterpolator = new ParameterMessageInterpolator();
    ...
    private static final class Internals {
        ...
        private static final ConstraintValidatorContext CONSTRAINT_VALIDATOR_CONTEXT = new InternalConstraintValidatorContext();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder CONSTRAINT_VIOLATION_BUILDER = new InternalConstraintViolationBuilder();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.NodeBuilderDefinedContext NODE_BUILDER_DEFINED_CONTEXT = new InternalNodeBuilderDefinedContext();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.NodeBuilderCustomizableContext NODE_BUILDER_CUSTOMIZABLE_CONTEXT = new InternalNodeBuilderCustomizableContext();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.LeafNodeBuilderCustomizableContext LEAF_NODE_BUILDER_CUSTOMIZABLE_CONTEXT = new InternalLeafNodeBuilderCustomizableContext();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.ContainerElementNodeBuilderCustomizableContext CONTAINER_ELEMENT_NODE_BUILDER_CUSTOMIZABLE_CONTEXT = new InternalContainerElementNodeBuilderCustomizableContext();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.NodeContextBuilder NODE_CONTEXT_BUILDER = new InternalNodeContextBuilder();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.LeafNodeContextBuilder LEAF_NODE_CONTEXT_BUILDER = new InternalLeafNodeContextBuilder();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.ContainerElementNodeContextBuilder CONTAINER_ELEMENT_NODE_CONTEXT_BUILDER = new InternalContainerElementNodeContextBuilder();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.LeafNodeBuilderDefinedContext LEAF_NODE_BUILDER_DEFINED_CONTEXT = new InternalLeafNodeBuilderDefinedContext();
        private static final ConstraintValidatorContext.ConstraintViolationBuilder.ContainerElementNodeBuilderDefinedContext CONTAINER_ELEMENT_NODE_BUILDER_DEFINED_CONTEXT = new InternalContainerElementNodeBuilderDefinedContext();
        ...
        private static final class InternalConstraintViolationBuilder implements ConstraintValidatorContext.ConstraintViolationBuilder {
            @Override
    ...
    private static final class InternalContext implements MessageInterpolator.Context {
    ...
    private static final class InternalConstraintDescriptor<T extends Annotation> implements ConstraintDescriptor<T> {
        private final T annotation;
        private final Map<String, Object> attributes;
        private final Class<? extends ConstraintValidator<T, ?>> validatorClass;
