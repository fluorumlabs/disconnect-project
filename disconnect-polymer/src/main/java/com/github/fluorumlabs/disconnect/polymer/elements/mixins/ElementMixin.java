...
public interface ElementMixin extends PropertyEffects, PropertiesMixin {
	/**
	 * When using the ShadyCSS scoping and custom property shim, causes all shimmed `styles` (via `custom-style`) in
	 * the
	 * document (and its subtree) to be updated based on current custom property values.
	 * <p>
	 ...
	 * <p>
	 * These properties are retained unless a value of `null` is set. FIXME param props: Object=
	 *
	 * @param props Bag of custom property key/values to apply to the document.
	 */
