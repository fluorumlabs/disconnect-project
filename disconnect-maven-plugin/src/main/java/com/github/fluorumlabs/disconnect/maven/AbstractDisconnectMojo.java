...
abstract class AbstractDisconnectMojo extends AbstractMojo {
    ...
    String getBuildTimestamp() {
        if (Globals.isLiveMode() || Globals.isTestMode()) {
            return "";
        }
        if (buildTimestamp == null) {
            buildTimestamp = "-" + Long.toString(System.currentTimeMillis(), 36);
        }
    ...
    void build(String compilationUnit, String entryPoint, boolean productionBuild) throws MojoExecutionException {
        ...
        tool.getProperties().setProperty("frontend.build", getBuildMode());
        ...
        executeMojo(
                ...
                configuration(
                        ...
                        element(name("environmentVariables"),
                                ...
                                element(name("NODE_BUILD_ID"), getBuildTimestamp()),
                                element(name("NODE_ENV"), getBuildMode())
                        ),
    ...
    void injectBuildTimestamp(File destination) throws MojoExecutionException {
        ...
        try {
            for (File file : FileUtils.listFiles(destination, new String[]{"html", "json"}, true)) {
                ...
                String replacedContent = StringUtils.replace(content, "${DISCONNECT_BUILD_ID}", getBuildTimestamp());
